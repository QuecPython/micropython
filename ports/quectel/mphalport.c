...
//Added by Freddy @20210520 sleep接口循环调用此接口达到sleep的目
//当有callback要执行时, 给此queue发
mp_uint_t qpy_mthread_sleep_deal_fun(mp_uint_t ms)
{
	if(mp_mthread_sleep_deal_is_inited())
	{
...
void _add_list_node()//头插法
{
    ...
    Head_node = node;
    //mp_hal_stdout_tx_str("_add_list_node\r\n");
}
//mia.zhong @20220308 fix input of multi thread dump problem.
void _delete_list_node()//delete node
{
    ...
    if (Head_node) {
        ...
        //mp_hal_stdout_tx_str("_delete_list_node\r\n");
    }
}

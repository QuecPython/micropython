...
static mp_obj_t qpy_misc_set_replEnable(size_t n_args, const mp_obj_t *args)
{
    ...
    if (strlen(g_repl_protect_pswd) > 0)
    {
        if (n_args > 1)
        {
            ...
            if ((bufinfo.len > 0) && (strcmp(g_repl_protect_pswd, bufinfo.buf) == 0)) /* 密码验证正确 */
            {
...
static mp_obj_t qpy_misc_set_repl_password(size_t n_args, const mp_obj_t *args)
{
    ...
    //mp_get_buffer_raise(old_password, &old_bufinfo, MP_BUFFER_READ);
    ...
    //mp_printf(&mp_plat_print, "pswd[%d]=%s\n g_repl_protect_pswd[%d]=%s\n", bufinfo.len,bufinfo.buf, strlen(g_repl_protect_pswd), g_repl_protect_pswd);
#endif    
	return mp_obj_new_int(0);
}

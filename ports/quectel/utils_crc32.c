...
//typedef uInt FAR uLongf;


#define local static
#define ZEXPORT	/* empty */
unsigned int crc32_v2 (unsigned int, const unsigned char *, unsigned int);

/* ========================================================================
 * Table of CRC-32's of all single-byte values (made by make_crc_table)
 */
local const uInt crc_table_v2[256] = {
...
/* ========================================================================= */
#define DO1(buf) crc = crc_table_v2[((int)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
#define DO2(buf)  DO1(buf); DO1(buf);
#define DO4(buf)  DO2(buf); DO2(buf);
#define DO8(buf)  DO4(buf); DO4(buf);

/* ========================================================================= */
uInt ZEXPORT crc32_v2(crc, buf, len)
    uInt crc;
    const Byte *buf;
    uInt len;
{
...
{
	crc32_obj_t *self = mp_obj_malloc_with_finaliser(crc32_obj_t, &mp_crc32_type);
...
static mp_obj_t crc32_update(mp_obj_t self, mp_obj_t crc32_init, mp_obj_t data)
{
    ...
    crc32_out = crc32_v2(crc32_in, bufinfo.buf, bufinfo.len);
...
MP_DEFINE_CONST_OBJ_TYPE(
    ...
    MP_TYPE_FLAG_NONE,
    make_new, crc32_make_new,
    locals_dict, &utils_crc32_locals_dict
    );
